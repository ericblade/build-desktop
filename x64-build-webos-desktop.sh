...
function build_qt4
{
    ...
    if [ ! -d $BASE/qt-build-desktop ] ; then
      ...
      if [ ! -e ../qt4/palm-desktop-configure.orig ] ; then
        ...
        sed -i 's/-opensource/-opensource -arch x86_64 -fast -qconfig palm -no-dbus/' ../qt4/palm-desktop-configure
...
function build_webkit
{
    ...
    # see https://bugs.webkit.org/show_bug.cgi?id=92264
    sed -i 's/namespace WebCore {/using namespace WebCore;/' Source/WebCore/xml/XPathParser.cpp
    sed -i 's/namespace XPath {/using namespace XPath;/' Source/WebCore/xml/XPathParser.cpp
    sed -i 'N;$!P;$!D;$d' Source/WebCore/xml/XPathParser.cpp
    
    sed -i 's/#include <stdlib.h>/#include <stdlib.h>\n#include <unistd.h>/' Source/WebCore/platform/webos/LunaServiceMgr.cpp
...
function build_luna-sysservice 
{
    ...
    sed -i 's/#include <string.h>/#include <string.h>\n#include <unistd.h>/' Src/PrefsDb.cpp
    sed -i 's/#include <stdarg.h>/#include <stdarg.h>\n#include <unistd.h>/' Src/Utils.cpp
    sed -i 's/lQtGui/lQtGui -lrt/' Makefile.inc
...
function build_mojomail
{
    ...
    sed -i 's!#include "glib/gbase64.h"!!' common/src/email/Rfc2047Decoder.cpp
    for SUBDIR in common imap pop smtp ; do
      mkdir -p $BASE/mojomail/mojomail/$SUBDIR/build
      cd $BASE/mojomail/mojomail/$SUBDIR/build
      sed -i 's!DESTINATION /!DESTINATION !' ../CMakeLists.txt
      sed -i 's!#pkg!pkg!' ../CMakeLists.txt
      sed -i 's!#inc!inc!' ../CMakeLists.txt
      sed -i 's!#web!web!' ../CMakeLists.txt
      sed -i 's!include(FindPkgConfig)!include(FindPkgConfig)\nwebos_add_compiler_flags(ALL "-fpermissive")!' ../CMakeLists.txt
      $CMAKE -D WEBOS_INSTALL_ROOT:PATH=${LUNA_STAGING} -DCMAKE_INSTALL_PREFIX=${LUNA_STAGING} ..
      #make $JOBS VERBOSE=1
      if [ "$SUBDIR" = "common" ]; then
        sed -i 's!std::min(wrap!std::min(static_cast<size_t>(wrap)!' ../src/util/Hexdump.cpp
      fi
      if [ "$SUBDIR" = "imap" ]; then
        sed -i 's!std::min(maxSize!std::min(static_cast<long unsigned int>(maxSize)!' ../src/sync/UIDMap.cpp
      fi
      make $JOBS
      make install
      mkdir -p $ROOTFS/usr/palm/public/accounts
...
function build_BrowserServer
{
    ...
    sed -i 's/-Werror//' Makefile.Ubuntu
    sed -i 's/glib-2.0 \\/glib-2.0 \\\n\t-ldl \\/' Makefile.Ubuntu
    sed -i 's/#include <stdlib.h>/#include <stdlib.h>\n#include <unistd.h>/' Src/PluginDirWatcher.cpp
    export QT_INSTALL_PREFIX=$LUNA_STAGING
    export STAGING_DIR=${LUNA_STAGING}
    export STAGING_INCDIR="${LUNA_STAGING}/include"
    export STAGING_LIBDIR="${LUNA_STAGING}/lib"
...
function build_node-addon
{
    ...
    if [ "${1}" = "sysbus" ] ; then
      sed -i 's/include(FindPkgConfig)/include(FindPkgConfig)\n\npkg_check_modules(CJSON REQUIRED cjson)\ninclude_directories(${CJSON_INCLUDE_DIRS})\nwebos_add_compiler_flags(ALL ${CJSON_CFLAGS_OTHER})/' ../CMakeLists.txt
    fi
    $CMAKE -D WEBOS_INSTALL_ROOT:PATH=${LUNA_STAGING} -DCMAKE_INSTALL_PREFIX=${LUNA_STAGING} ..
    make $JOBS VERBOSE=1
    #make $JOBS
...
function build_db8
{
    ...
    cd $BASE/db8
    sed -i 's/-ldb-4.8/-ldb/' build/Makefile.inc
    sed -i 's/-Werror/-fpermissive/' build/Makefile.inc
    make $JOBS -e PREFIX=$LUNA_STAGING -f Makefile.Ubuntu install BUILD_TYPE=release
    # NOTE: Make binary findable in /usr/lib/luna so ls2 can match the role file
...
function build_configurator
{
    ...
    sed -i 's/-Werror//' Makefile.Ubuntu
    ARCH_LDFLAGS="-Wl,-rpath-link $LUNA_STAGING/lib `pkg-config --libs glib-2.0`" make $JOBS -f Makefile.Ubuntu
    # NOTE: Make binary findable in /usr/lib/luna so ls2 can match the role file
